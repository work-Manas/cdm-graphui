...
export function buildGpuInference(): Architecture {
  ...
  const edges = [
    makeFlowEdge({
      ...
      ports: [{ protocol: "HTTPS", port: 443 }],
      label: "Enqueue :443",
      throughput: { current: 720, peak: 1200 },
      status: "active",
    }),
    makeFlowEdge({
      source: apiGw.id,
      ...
      throughput: { current: 320, peak: 880 },
      status: "active",
    }),
    makeFlowEdge({
      source: apiGw.id,
      ...
      ports: [{ protocol: "gRPC", port: 8470 }],
      label: "gRPC :8470 (sync)",
      throughput: { current: 240, peak: 720 },
      status: "degraded",
    }),
    makeFlowEdge({
      source: sqs.id,
      ...
      ports: [{ protocol: "gRPC", port: 8470 }],
      label: "Async dequeue",
      throughput: { current: 280, peak: 620 },
      status: "active",
    }),
    makeFlowEdge({
      source: sqs.id,
    ...
    makeFlowEdge({
      ...
      throughput: { current: 12, peak: 380 },
      status: "idle",
    }),
    makeFlowEdge({
      source: h100B.id,
    ...
    makeFlowEdge({
      ...
      ports: [{ protocol: "HTTPS", port: 443 }],
      label: "Cache write",
      throughput: { current: 240, peak: 540 },
      status: "active",
    }),
    makeFlowEdge({
      source: h100B.id,
      ...
      ports: [{ protocol: "HTTPS", port: 443 }],
      label: "Cache write",
      throughput: { current: 140, peak: 480 },
      status: "degraded",
    }),
  ];
